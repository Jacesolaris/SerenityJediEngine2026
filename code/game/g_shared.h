...
struct gentity_s
{
	...
	gclient_t* client = nullptr; // NULL if not a player (unless it's NPC ( if (this->NPC != nullptr)  )  <sigh>... -slc)
	...
	vec3_t absmin = { 0, 0, 0 }, absmax = { 0, 0, 0 }; // derived from mins/maxs and origin + rotation

	// currentOrigin will be used for all collision detection and world linking.
	// it will not necessarily be the same as the trajectory evaluation for the current
	// time, because each entity must be moved one at a time after time is advanced
	// to avoid simultanious collision issues
	vec3_t currentOrigin = { 0, 0, 0 };
	vec3_t currentAngles = { 0, 0, 0 };
	...
	*/
	...
	int sentryDeadThink = 0;
	int forceFieldThink = 0;
	...
	vec3_t lastOrigin = { 0, 0, 0 }; //Where you were last frame
	vec3_t lastAngles = { 0, 0, 0 }; //Where you were looking last frame
