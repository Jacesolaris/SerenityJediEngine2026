...
static saberMoveName_t PM_PickBackStab()
{
	if (!pm->gent || !pm->gent->client)
	{
		return LS_READY;
...
static saberMoveName_t PM_CheckStabDown()
{
...
saberMoveName_t PM_SaberFlipOverAttackMove()
{
	// Check for overridden (or cancelled) kata move
	if (pm->ps->saber[0].jumpAtkFwdMove != LS_INVALID)
	{
		if (pm->ps->saber[0].jumpAtkFwdMove != LS_NONE)
		{
	...
	// Adjust jump height based on enemy size and height difference
	if (pm->gent &&
		pm->gent->enemy &&
		pm->gent->enemy->inuse)
	{
		// Taller enemies → higher jump
		pm->ps->velocity[2] *= (pm->gent->enemy->maxs[2] - pm->gent->enemy->mins[2]) / 64.0f;
	...
	// Safe pointer chain check for sound selection
	if (pm->gent &&
		pm->gent->client &&
		pm->gent->client->ps.forcePowerLevel[FP_LEVITATION] < FORCE_LEVEL_3)
	{
	...
	if (pm->gent)
	{
		pm->gent->angle = pm->ps->viewangles[YAW];
	}
...
static saberMoveName_t PM_CheckStaffKata()
{
	if (pm->ps->clientNum < MAX_CLIENTS
		&& PM_InSecondaryStyle())
	{
		...
		if (forwardmove > 0)
		{
				...
				&& pm->gent->enemy->client)
			{
				...
				if (pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer())
				{
					...
					{
						const saberMoveName_t auto_move = PM_AttackForEnemyPos(qfalse,
	...
	const int fatigued)
{
	...
	//======================================================================
	if (gent->client->ps.stats[STAT_WEAPONS] & 1 << WP_SCEPTER &&
		...
		if (Q_stricmp(level.mapname, "t2_wedge") != 0)
		{
			...
			if (gi.WE_GetWindVector(wind_dir, ent->currentOrigin))
			{
				...
				if (DotProduct(fwd, wind_dir) > 0.65f)
				{
					...
					{
						if (!((ent->client->ps.ManualBlockingFlags & 1 << MBF_HOLDINGBLOCK) != 0) &&
							!(pm->ps->PlayerEffectFlags & 1 << PEF_SPRINTING) &&
							!(pm->ps->PlayerEffectFlags & 1 << PEF_WEAPONSPRINTING))
						{
							if (ent->client && ent->client->ps.torsoAnim != BOTH_WIND)
	...
	else if (pm->ps->weaponstate == WEAPON_IDLE)
	{
		...
		else
		{
			...
			if (pm->ps->saberInFlight)
			{
				...
				int saberEntNum = pm->ps->saberEntityNum;

				if (saberEntNum > 0 &&
					saberEntNum < ENTITYNUM_WORLD &&
					g_entities[saberEntNum].inuse &&
					g_entities[saberEntNum].s.pos.trType == TR_STATIONARY)
				{
...
void PM_TorsoAnimation()
{
	...
	// this is the ammount of time the gun is considered busy and stays in position after the last shot was fired
	...
	else if (pm->ps->lastShotTime > level.time - 2500 && cg.renderingThirdPerson && PM_IsPistoleer() && (!(pm->ps->communicatingflags & (1u << CF_AIMINGGUN))))
	{
		weaponBusy = qtrue;
	}
	else if (pm->ps->lastShotTime > level.time - 1000 && cg.renderingThirdPerson && PM_IsPistoleer() && (pm->ps->communicatingflags & (1u << CF_AIMINGGUN)))
	{
		weaponBusy = qtrue;
	}
	else if (pm->ps->lastShotTime > level.time - 3000 && cg.renderingThirdPerson && (!(pm->ps->communicatingflags & (1u << CF_AIMINGGUN))))
	{
		weaponBusy = qtrue;
	}
	else if (pm->ps->lastShotTime > level.time - 1000 && cg.renderingThirdPerson && (pm->ps->communicatingflags & (1u << CF_AIMINGGUN)))
	{
		weaponBusy = qtrue;
	}
	...
	{ //if the weapon is idle, and not in a saber bounce, return to a stand anim
