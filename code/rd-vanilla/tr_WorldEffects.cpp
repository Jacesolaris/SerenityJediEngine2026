...
public:
	...
	void			AddWeatherZone(vec3_t mins, vec3_t maxs)
	{
		...
		if (!mWeatherZones.full())
		{
			...
			Wz.miPointCacheByteSize = (static_cast<unsigned long long>(Wz.mWidth) * Wz.mHeight * Wz.mDepth) * sizeof(uint32_t);
